...
const Cancel_Appointment_Detail = ({navigation, route}) => {
  ...
  const _cancellAppointmentApi = () => {
    ...
    PostAPiwithToken({url: 'cancelledstatus', Token: user?.api_token}, formdata)
      .then(res => {
        if (res.status == 'success') {
          ToastAndroid.show('Cancelled Successfully!', ToastAndroid.SHORT);
        ...
        <ScrollView>
          ...
          <View style={{marginTop: wp(20), marginBottom: wp(6)}}>
            <Button
              ...
              onPress={_cancellAppointmentApi}
            />
          </View>
        </ScrollView>
      </View>
...
export default Cancel_Appointment_Detail;
