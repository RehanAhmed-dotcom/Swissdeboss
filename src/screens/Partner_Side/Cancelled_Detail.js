...
const Cancelled_Detail = ({navigation, route}) => {
  ...
  // console.log('item on cancel screeeen', item);
  ...
  const _FetchAppointmentsAPiPending = () => {
    ...
    AllGetAPI({url: 'cancel_app', Token: user?.api_token})
      .then(res => {
        // setIsLoading(false);
        setPendingData(res.canceled_appointments);
        console.log('Appointment Data _Fetch   cancel_app./././././.', res);
      })
      .catch(err => {
        // setIsLoading(false);
...
export default Cancelled_Detail;
