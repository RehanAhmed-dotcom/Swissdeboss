...
const Accepted_Detail = ({navigation}) => {
  ...
  const _FetchAppointmentsAPiPending = () => {
    ...
    AllGetAPI({url: 'active_app', Token: user?.api_token})
      .then(res => {
        // setIsLoading(false);
        setPendingData(res.active_appointments);
        console.log('Appointment Data _Fetch   accepted./././././.', res);
      })
      .catch(err => {
        // setIsLoading(false);
...
export default Accepted_Detail;
